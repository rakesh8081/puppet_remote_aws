...
git clone https://github.com/rakesh8081/puppet_remote_aws && cd puppet_remote_aws && docker build -t puppet_remote_aws/local:apache .

docker run -d -p 80:80 --name apache --link mysql:mysql -v /var/www/html:/var/www/html puppet_remote_aws/local:apache
